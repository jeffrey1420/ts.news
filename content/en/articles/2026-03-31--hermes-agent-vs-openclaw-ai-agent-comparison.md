...
The technical architecture is built around a modular Python codebase with distinct subsystems: the agent loop (`AIAgent` in `run_agent.py`), a prompt assembly and compression system, a provider runtime resolver supporting [OpenRouter](/articles/2026-06-14-openrouter-fusion-compound-ai-draco), OpenAI, Anthropic, Nous Portal, z.ai/GLM, [Kimi](/articles/2026-06-12-kimi-k2-7-code-mimo-code)/Moonshot, and MiniMax, a tools runtime with 40+ built-in tools, a messaging gateway for platform adapters, and an RL/environments subsystem for trajectory generation and training data.
